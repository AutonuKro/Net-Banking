#include <stdio.h>
#include <sys/types.h> 
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h> 
#include <stdlib.h>
#include <sys/sendfile.h>
#include <string.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <sys/wait.h>

#define MAX 1024

char *client_ip;
int client_port;

extern void customer(int,int);
extern void police(int);
extern void admin(int);

int verify_credentials(int sock_fd, char *user_type,int *cust_id)
{
    int n;
    char temp[MAX];
    char *user, *pass;

    // reading username and password
    memset(temp,0,MAX); 
    n = read(sock_fd,temp,MAX-1);
    if (n < 0) 
        error("ERROR reading from socket");
    
    // breaking it
    user = strtok(temp, "$$$");
    pass = strtok(NULL, "$$$"); 
       
    user[strlen(user)-1] = '\0';
    pass[strlen(pass)-1] = '\0';

    if(strlen(user)==0 || strlen(pass)==0)
        return 0;

    // checking for validity
    FILE *fp = fopen("login_file.txt","r");
    if(fp == NULL)
        error("Error in opening login_file.");
    
    char *cred = NULL;
    size_t len = 0;
    
    while(getline(&cred,&len,fp)!=-1)
    {
        char *username = strtok(cred," ");
        char *password = strtok(NULL," ");
        char *usertype = strtok(NULL, " ");
        if(!strcmp(username,user)&&!strcmp(password,pass))
        {
            *user_type = usertype[0];
            *cust_id = atoi(username);
            free(cred);
            fclose(fp);
            return 1;
        } 
    }
    fclose(fp);
    free(cred);
    return 0;
}


void service_provider(int sock_fd)
{
    int n;
    char temp[MAX];
    
    int count=1;
    char user_type;
    int cust_id;
    
    while(!verify_credentials(sock_fd,&user_type,&cust_id))
    {
        if(count>=3)
        {
            // sending exit
            memset(temp,0,MAX);
            strcpy(temp,"exit");
            n = write(sock_fd,temp,strlen(temp));
            return;
        }

        fprintf(stdout, "Verification for client with ip address '%s' failed. \n", client_ip);            
        // sending false
        memset(temp,0,MAX);
        strcpy(temp,"false");
        n = write(sock_fd,temp,strlen(temp));
        if (n < 0)
        {
            perror("ERROR writing to socket");
            exit(1);
        } 
        count++;
    }
    
    fprintf(stdout, "Verification for client with ip address '%s' successful. \n", client_ip);            
    // sending user type
    memset(temp,0,MAX);
    temp[0] = user_type;
    temp[1] = '\0';
    n = write(sock_fd,temp,strlen(temp));
    if (n < 0)
    {
        perror("ERROR writing to socket");
        exit(1);
    } 
    // calling corresponding function
    if(user_type == 'C')
        customer(sock_fd,cust_id);
    else if(user_type == 'A')
        admin(sock_fd);
    else if(user_type == 'P')
        police(sock_fd);

    return;
}



int main(int argc, char *argv[])
{
    int sock_fd, clientfd, port, client_len;
    int pid, process_id;
    int child_count = 0;
    int enable = 1;
    
    struct sockaddr_in server_addr, client_addr;
    
    if (argc < 2) 
    {
        fprintf(stderr,"ERROR: no port is provided.\n");
        exit(1);
    }
    if ((sock_fd = socket(AF_INET, SOCK_STREAM,IPPROTO_TCP))== -1)
    {
        perror("ERROR in opening socket");
        exit(1);
    }
    
    // if (sock_fd < 0) 
    //     error("ERROR in opening socket");
    
    memset((char *) &server_addr,0, sizeof(server_addr));
    
    //get the integer value of the port.
    port = atoi(argv[1]);
    
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = htonl(INADDR_ANY);
    server_addr.sin_port = htons(port);
    
    if (setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, &enable, sizeof(int))== -1)
    {
        perror("Setting of socket as Reusable failed.");
        exit(1);
    }

    // binding socket to address
    if (bind(sock_fd, (struct sockaddr *) &server_addr, sizeof(server_addr))== -1)
    {
        perror("ERROR in binding.");
        exit(1);
    }     
    fprintf(stdout, "Binding done.\n");

    // socket listening with queue 5
    listen(sock_fd,5);
    fprintf(stdout, "Listening started with queue length 5.\n");

    client_len = sizeof(client_addr);
    while (1) 
    {
    	// accepting client request
        clientfd = accept(sock_fd, (struct sockaddr *) &client_addr, &client_len);
        if (clientfd == -1)
        {
            perror("ERROR in accepting connection.");
            exit(1);
        } 
        pid = fork();
        if (pid < 0)
        {
            perror("ERROR in forking.");
            exit(1);
        }
        if (pid == 0)  
        {
            close(sock_fd);
            
            client_ip = inet_ntoa(client_addr.sin_addr);
            client_port = ntohs(client_addr.sin_port);
            fprintf(stdout, "Connection accepted for client with ip address '%s' on port '%d'. \n", client_ip, client_port);
            
            // handling client
            service_provider(clientfd);
            
            close(clientfd);
            fprintf(stdout, "Connection closed for client with ip address '%s' on port '%d'. \n", client_ip, client_port);
            
            exit(0);
        }
        else 
        {
        	// closing client
            close(clientfd);
            // handling zombie process
            while (child_count) 
            {
                // Non-blocking wait 
                process_id = waitpid((pid_t) -1, NULL, WNOHANG); 
                if (process_id < 0) 
                    error("Error in cleaning zombie process.");
                else if(process_id == 0) 
                    break; 
                else 
                    child_count--; 
            }
        }
    } 
     return 0; 
}